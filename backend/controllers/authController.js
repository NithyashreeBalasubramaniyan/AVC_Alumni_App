...
const registeralumni = async (req, res) => {
  try {
    ...
    const { name, reg_no, ph_no, dob, password, mail, department } = req.body; // 👈 include department
    if (typeof reg_no !== 'string' || reg_no.length !== 12) 
      return res.status(400).json({ success:false, message:'Registration number must be 12 chars' });
    ...
    const alumni = await prisma.alumni.create({ 
      data:{ name, reg_no, ph_no, mail, dob: new Date(dob), password: hashedPassword, department } // 👈 must add department
    });
